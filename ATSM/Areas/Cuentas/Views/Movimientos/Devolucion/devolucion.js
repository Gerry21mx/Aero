var tabla, tant, tfon, devolucion, cuenta, cuentas;
$(() => {
	let id = getPar('id');
	let m = getPar('m');
...
function reg(b) {
	let tm = b.target.id === 'regf' ? 3 : 4;
	procc();
	if (!cuenta.Valid) {
		MsjBox('Cuenta No Valida', 'Valide la Cuenta Por Favor, ya que no es Valida.');
		cle();
	}
	devolucion.read('', tm === 3 ? 'nav-fondo' :'nav-anticipo');
	devolucion.IdAccount = cuenta.Id;
	devolucion.IdMovimiento = tm;
	devolucion.IdTipo = act;
	let rs = devolucion.save();
	MsjBox('Cuentas', rs.Error === '' ? rs.Mensaje : rs.Error);
	if (rs.Valid) {
		updDT(tm === 3 ? tfon : tant, devolucion, "Id");
		$(`#${(tm === 3 ? 'nav-fondo' : 'nav-anticipo')} input`).val('');
		$(`#${(tm === 3 ? 'nav-fondo' : 'nav-anticipo')} select`).val(0);
	}
	endPro();
}
function cle() {
	...
	<button tipo="anticipo" type="button" class="btn btn btn-sm btn-success"><i class="fas fa-money-check-alt"></i> Anticipo</button>
		...
		drawCallback: function (settings) {
			$(`#tabla button`).click((btn) => {
				...
				$(`#nav-${t}-tab`).tab('show');
				let dfon = Transaccion.getTransacciones(cuenta.Id, 3);
				let dant = Transaccion.getTransacciones(cuenta.Id, 4);
				loadTable(tfon, dfon);
				loadTable(tant, dant);
				//endPro();
			});
	...
	tfon = $('#tfon').DataTable({
		...
		drawCallback: function (settings) {
			$(`#tfon button`).click((btn) => {
				...
				procc();
				devolucion.clear();
				devolucion.setValores(data);
				devolucion.write('', 'nav-fondo');
				endPro();
			});
		}
	});
