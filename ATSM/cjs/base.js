class base {
	...
	async getInstance(iApi) {
		...
		if (res.ok) {
			...
			endPro();
			return { Valid: false, Mensaje: '', Error: `Error Fetched (${_this.constructor.name}.getInstance()): ${res.status}<br>${res.statusText}`, Data: null };
		}
	}
	async save() {
		procc();
		let res = await fetch(this._apiElement, { method: 'POST', body: JSON.stringify(this), headers: { 'Content-Type': 'application/json' });
		if (res.ok) {
			let post = await res.json();
			if (post !== null) {
				res = data;
			...
			console.log(`Error Fetched: ${res.status} - ${res.statusText}`);
			endPro();
			return { Valid: false, Mensaje: '', Error: `Error Fetched (${_this.constructor.name}.save()): ${res.status}<br>${res.statusText}`, Data: null };
		}
	}
	async delete() {
		procc();
